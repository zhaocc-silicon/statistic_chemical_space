...
class read_db():
    ...
    def get_mole_info(self,inchi_key):   
        con=pymysql.connect(
        host='10.86.1.149',   # 连接你要取出数据库的ip，如果是本机可以不用写   
        user='stx',     # 你的数据库用户名
        passwd='linux123',# 你的数据库密码
        ...
        cursor=con.cursor()
        #execute执行一句查询语句
        sql ="SELECT inchi_key, ring_number, heavy_atoms from compound_table WHERE (inchi_key = '%s') " %inchi_key
        #print(sql)
        cursor.execute(sql)
        try:
            num=cursor.fetchone()
    ...
    def get_compond_info(self):
        ...
        cursor=con.cursor()
        #execute执行一句查询语句
        sql ="SELECT DISTINCT inchi_key, smiles, function_group, ring_number, ring_size, ring_property, source  from compound_table WHERE source = 'ChEMBL_Drugs'"
        #sql ="SELECT DISTINCT inchi_key, smiles, function_group, ring_number, ring_size, ring_property, source  from compound_table WHERE frag_level != 'None'"
        cursor.execute(sql)
    ...
    def get_chembl(self, inchi_key):
        ...
        return num 
    '''
    def get_chembl_drugs_compound(self):
        con=pymysql.connect(
        host='10.86.1.149',   # 连接你要取出数据库的ip，如果是本机可以不用写   
        user='stx',     # 你的数据库用户名
        passwd='linux123',# 你的数据库密码
        ...
        cursor=con.cursor()
        #execute执行一句查询语句
        sql ="SELECT inchi_key from compound_table WHERE source = 'ChEMBL_Drugs' "
        cursor.execute(sql)
       # moles = cursor.fetchall()[0:50]
        moles = cursor.fetchall()
        return moles
    def get_compound_unique_key(self,inchi_key):
        con=pymysql.connect(
        host='10.86.1.149',   # 连接你要取出数据库的ip，如果是本机可以不用写   
        user='stx',     # 你的数据库用户名
        passwd='linux123',# 你的数据库密码
        ...
        charset='utf8')
        cursor=con.cursor()
        sql = "SELECT uniq_key from fragments_info WHERE frag_level='elementary_fragment' and inchi_key = '%s'"%inchi_key
        cursor.execute(sql)
        ele_key = cursor.fetchone()[0]
        
        cursor1=con.cursor()
        sql = "SELECT uniq_key from fragments_info WHERE frag_level='secondary_fragment' and inchi_key = '%s'"%inchi_key
        cursor1.execute(sql)
        sec_key = cursor1.fetchone()[0]
        
        return ele_key, sec_key
    
    def get_comp_frag_inchi(self,unique):
        con=pymysql.connect(
        host='10.86.1.149',   # 连接你要取出数据库的ip，如果是本机可以不用写   
        user='stx',     # 你的数据库用户名
        passwd='linux123',# 你的数据库密码
        ...
        charset='utf8')
        cursor=con.cursor()
        sql ="SELECT DISTINCT inchi_key from fragments_detail WHERE uniq_id = '%s'"%unique
        cursor.execute(sql)
        ele_keys = cursor.fetchall()
        return ele_keys
if __name__ == "__main__":
    ...
    #n,m = rd.get_compound_unique_key('CZPWVGJYEJSRLH-UHFFFAOYSA-N')
    num = rd.get_mole_info('IKHGUXGNUITLKF-UHFFFAOYSA-N')
    print(num)
    #print(num)
